...
int (*compare)(void * data1, void * data2,int sort) = NULL;
 ...
 */
LISTITEM_T * findMinimum(LISTITEM_T ** pPredecessor,int sort)
{
   ...
   *pPredecessor = NULL;
   if (count == 1)
	 ...
	 /* if this is the first item in the list, or
          ...
          */
	 if ((pMinItem == NULL) ||
	     ((*compare)(pCurrentItem->data,pMinItem->data,sort) < 0))
	    {
 ...
 * Argument - sort - the selected sorting criteria.
...
void * dequeueMin(int sort)
{
   ...
   if (count > 0)
      ...
      LISTITEM_T* pMinItem = findMinimum(&pPredItem,sort);
